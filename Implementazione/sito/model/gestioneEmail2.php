  ...
  try{
    ...
    while($row=$query->fetch(PDO::FETCH_ASSOC)){
      $query1 = $conn->prepare("SELECT for_email AS 'email',for_nome AS 'nome',
              (SELECT count(for_email) from gruefor where grue_id=:id AND for_email=:email) AS 'presente' from formatore where for_flag=1 AND for_email=:email1 group by for_nome");
